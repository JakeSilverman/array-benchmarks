(set-info :original "/var/folders/xz/sgqbj6yn0yv_1mpy6_jqympm0000gn/T/sea-Rx3wsj/sanfoundry_02_ground.pp.ms.o.bc")
(set-info :authors "SeaHorn v.0.1.0-rc3")
(declare-rel verifier.error (Bool Bool Bool ))
(declare-rel __JVERIFIER_assert@_call (Int ))
(declare-rel __JVERIFIER_assert@_ret (Int ))
(declare-rel __JVERIFIER_assert (Bool Bool Bool Int ))
(declare-rel main@entry (Int ))
(declare-rel main@_bb (Int Int Int (Array Int Int) ))
(declare-rel main@_bb4 (Int (Array Int Int) Int Int Int ))
(declare-rel main@_bb10 (Int (Array Int Int) Int Int Int ))
(declare-rel main@_bb13 (Int (Array Int Int) Int Int Int ))
(declare-rel main@verifier.error.split ())
(declare-var __JVERIFIER_assert@%_call1_0 Bool )
(declare-var __JVERIFIER_assert@arg.0_0 Int )
(declare-var __JVERIFIER_assert@_call_0 Bool )
(declare-var __JVERIFIER_assert@_ret_0 Bool )
(declare-var main@%_5_0 Int )
(declare-var @__VERIFIER_nondet_int_0 Int )
(declare-var main@%_6_0 Int )
(declare-var main@%_7_0 Int )
(declare-var main@%_24_0 Int )
(declare-var main@%_22_0 Bool )
(declare-var main@%_18_0 Int )
(declare-var main@%_20_0 Bool )
(declare-var main@%.27.i_3 Int )
(declare-var main@%.2.i_3 Int )
(declare-var main@%_54_0 Bool )
(declare-var main@%_32_0 Int )
(declare-var main@%_33_0 Int )
(declare-var main@%_34_0 Bool )
(declare-var main@%_36_0 Bool )
(declare-var main@%_52_0 Bool )
(declare-var main@%_46_0 Int )
(declare-var main@%_47_0 Int )
(declare-var main@%_42_0 Int )
(declare-var main@%_43_0 Int )
(declare-var main@%_44_0 Bool )
(declare-var main@%_50_2 Bool )
(declare-var main@%_40_0 Bool )
(declare-var main@%_30_0 Bool )
(declare-var main@%_16_0 Bool )
(declare-var main@%_10_0 Int )
(declare-var main@%_11_0 Int )
(declare-var main@%_12_0 Int )
(declare-var main@%_13_0 Int )
(declare-var main@%_14_0 Bool )
(declare-var main@%_3_0 Bool )
(declare-var main@%_1_0 Int )
(declare-var main@entry_0 Bool )
(declare-var main@%sm1_0 (Array Int Int) )
(declare-var main@%_0_0 Int )
(declare-var main@_bb_0 Bool )
(declare-var main@%shadow.mem.0.0_0 (Array Int Int) )
(declare-var main@%.01.i_0 Int )
(declare-var main@%shadow.mem.0.0_1 (Array Int Int) )
(declare-var main@%.01.i_1 Int )
(declare-var main@_bb2_0 Bool )
(declare-var main@%sm_0 (Array Int Int) )
(declare-var main@%_8_0 Int )
(declare-var main@_bb_1 Bool )
(declare-var main@%shadow.mem.0.0_2 (Array Int Int) )
(declare-var main@%.01.i_2 Int )
(declare-var main@_bb3_0 Bool )
(declare-var main@%..i_0 Int )
(declare-var main@%.8.i_0 Int )
(declare-var main@_bb4_0 Bool )
(declare-var main@%.16.i_0 Int )
(declare-var main@%.14.i_0 Int )
(declare-var main@%.12.i_0 Int )
(declare-var main@%.16.i_1 Int )
(declare-var main@%.14.i_1 Int )
(declare-var main@%.12.i_1 Int )
(declare-var main@_bb5_0 Bool )
(declare-var main@%_19_0 Int )
(declare-var main@_bb6_0 Bool )
(declare-var main@_bb7_0 Bool )
(declare-var main@%_25_0 Int )
(declare-var main@_bb8_0 Bool )
(declare-var |tuple(main@_bb6_0, main@_bb8_0)| Bool )
(declare-var |tuple(main@_bb5_0, main@_bb8_0)| Bool )
(declare-var main@%.27.i_0 Int )
(declare-var main@%.2.i_0 Int )
(declare-var main@%.27.i_1 Int )
(declare-var main@%.2.i_1 Int )
(declare-var main@%.27.i_2 Int )
(declare-var main@%.2.i_2 Int )
(declare-var main@%_27_0 Int )
(declare-var main@_bb4_1 Bool )
(declare-var main@%.16.i_2 Int )
(declare-var main@%.14.i_2 Int )
(declare-var main@%.12.i_2 Int )
(declare-var main@_bb9_0 Bool )
(declare-var main@_bb10_0 Bool )
(declare-var main@%.0.i_0 Int )
(declare-var main@%.0.i_1 Int )
(declare-var main@_bb11_0 Bool )
(declare-var main@%_35_0 Int )
(declare-var main@postcall_0 Bool )
(declare-var main@%_37_0 Int )
(declare-var main@_bb10_1 Bool )
(declare-var main@%.0.i_2 Int )
(declare-var main@_bb12_0 Bool )
(declare-var main@_bb13_0 Bool )
(declare-var main@%.1.i_0 Int )
(declare-var main@%.1.i_1 Int )
(declare-var main@precall_0 Bool )
(declare-var main@__JVERIFIER_assert_0 Bool )
(declare-var main@%.0_0 Int )
(declare-var main@%.0_1 Int )
(declare-var main@_bb17_0 Bool )
(declare-var main@verifier.error_0 Bool )
(declare-var main@verifier.error.split_0 Bool )
(declare-var main@_bb14_0 Bool )
(declare-var main@_bb15_0 Bool )
(declare-var main@%_48_0 Bool )
(declare-var main@_bb16_0 Bool )
(declare-var |tuple(main@_bb14_0, main@_bb16_0)| Bool )
(declare-var main@%_50_0 Bool )
(declare-var main@%_50_1 Bool )
(declare-var main@%_51_0 Int )
(declare-var main@postcall1_0 Bool )
(declare-var main@%_53_0 Int )
(declare-var main@_bb13_1 Bool )
(declare-var main@%.1.i_2 Int )
(declare-var main@precall2_0 Bool )
(rule (verifier.error false false false))
(rule (verifier.error false true true))
(rule (verifier.error true false true))
(rule (verifier.error true true true))
(rule (__JVERIFIER_assert true true true __JVERIFIER_assert@arg.0_0))
(rule (__JVERIFIER_assert false true true __JVERIFIER_assert@arg.0_0))
(rule (__JVERIFIER_assert false false false __JVERIFIER_assert@arg.0_0))
(rule (__JVERIFIER_assert@_call __JVERIFIER_assert@arg.0_0))
(rule (=> (and (__JVERIFIER_assert@_call __JVERIFIER_assert@arg.0_0)
         true
         (= __JVERIFIER_assert@%_call1_0 (= __JVERIFIER_assert@arg.0_0 0))
         (not __JVERIFIER_assert@%_call1_0)
         (=> __JVERIFIER_assert@_ret_0
             (and __JVERIFIER_assert@_ret_0 __JVERIFIER_assert@_call_0))
         __JVERIFIER_assert@_ret_0)
    (__JVERIFIER_assert@_ret __JVERIFIER_assert@arg.0_0)))
(rule (=> (__JVERIFIER_assert@_ret __JVERIFIER_assert@arg.0_0)
    (__JVERIFIER_assert true false false __JVERIFIER_assert@arg.0_0)))
(rule (main@entry @__VERIFIER_nondet_int_0))
(rule (=> (and (main@entry @__VERIFIER_nondet_int_0)
         true
         (> main@%_0_0 0)
         (= main@%_1_0 main@%_0_0)
         (=> main@_bb_0 (and main@_bb_0 main@entry_0))
         (=> (and main@_bb_0 main@entry_0)
             (= main@%shadow.mem.0.0_0 main@%sm1_0))
         (=> (and main@_bb_0 main@entry_0) (= main@%.01.i_0 0))
         (=> (and main@_bb_0 main@entry_0)
             (= main@%shadow.mem.0.0_1 main@%shadow.mem.0.0_0))
         (=> (and main@_bb_0 main@entry_0) (= main@%.01.i_1 main@%.01.i_0))
         main@_bb_0)
    (main@_bb @__VERIFIER_nondet_int_0
              main@%_0_0
              main@%.01.i_1
              main@%shadow.mem.0.0_1)))
(rule (let ((a!1 (=> main@_bb2_0
               (= main@%_7_0 (+ main@%_0_0 (* 0 400000) (* main@%.01.i_0 4))))))
(let ((a!2 (and (main@_bb @__VERIFIER_nondet_int_0
                          main@%_0_0
                          main@%.01.i_0
                          main@%shadow.mem.0.0_0)
                true
                (= main@%_3_0 (< main@%.01.i_0 100000))
                (=> main@_bb2_0 (and main@_bb2_0 main@_bb_0))
                (=> (and main@_bb2_0 main@_bb_0) main@%_3_0)
                (=> main@_bb2_0 (= main@%_5_0 @__VERIFIER_nondet_int_0))
                a!1
                (=> main@_bb2_0 (or (<= main@%_0_0 0) (> main@%_7_0 0)))
                (=> main@_bb2_0 (> main@%_0_0 0))
                (=> main@_bb2_0
                    (= main@%sm_0
                       (store main@%shadow.mem.0.0_0 main@%_7_0 main@%_6_0)))
                (=> main@_bb2_0 (= main@%_8_0 (+ main@%.01.i_0 1)))
                (=> main@_bb_1 (and main@_bb_1 main@_bb2_0))
                (=> (and main@_bb_1 main@_bb2_0)
                    (= main@%shadow.mem.0.0_1 main@%sm_0))
                (=> (and main@_bb_1 main@_bb2_0) (= main@%.01.i_1 main@%_8_0))
                (=> (and main@_bb_1 main@_bb2_0)
                    (= main@%shadow.mem.0.0_2 main@%shadow.mem.0.0_1))
                (=> (and main@_bb_1 main@_bb2_0)
                    (= main@%.01.i_2 main@%.01.i_1))
                main@_bb_1)))
  (=> a!2
      (main@_bb @__VERIFIER_nondet_int_0
                main@%_0_0
                main@%.01.i_2
                main@%shadow.mem.0.0_2)))))
(rule (let ((a!1 (= main@%_10_0 (+ (+ main@%_0_0 (* 0 400000)) (* 0 4))))
      (a!2 (= main@%_12_0 (+ (+ main@%_0_0 (* 0 400000)) (* 1 4)))))
(let ((a!3 (and (main@_bb @__VERIFIER_nondet_int_0
                          main@%_0_0
                          main@%.01.i_0
                          main@%shadow.mem.0.0_0)
                true
                (= main@%_3_0 (< main@%.01.i_0 100000))
                (=> main@_bb3_0 (and main@_bb3_0 main@_bb_0))
                (=> (and main@_bb3_0 main@_bb_0) (not main@%_3_0))
                (=> main@_bb3_0 a!1)
                (=> main@_bb3_0 (or (<= main@%_0_0 0) (> main@%_10_0 0)))
                (=> main@_bb3_0
                    (= main@%_11_0 (select main@%shadow.mem.0.0_0 main@%_10_0)))
                (=> main@_bb3_0 a!2)
                (=> main@_bb3_0 (or (<= main@%_0_0 0) (> main@%_12_0 0)))
                (=> main@_bb3_0 (> main@%_0_0 0))
                (=> main@_bb3_0
                    (= main@%_13_0 (select main@%shadow.mem.0.0_0 main@%_12_0)))
                (=> main@_bb3_0 (= main@%_14_0 (< main@%_11_0 main@%_13_0)))
                (=> main@_bb3_0
                    (= main@%..i_0 (ite main@%_14_0 main@%_11_0 main@%_13_0)))
                (=> main@_bb3_0
                    (= main@%.8.i_0 (ite main@%_14_0 main@%_13_0 main@%_11_0)))
                (=> main@_bb4_0 (and main@_bb4_0 main@_bb3_0))
                (=> (and main@_bb4_0 main@_bb3_0) (= main@%.16.i_0 main@%..i_0))
                (=> (and main@_bb4_0 main@_bb3_0)
                    (= main@%.14.i_0 main@%.8.i_0))
                (=> (and main@_bb4_0 main@_bb3_0) (= main@%.12.i_0 2))
                (=> (and main@_bb4_0 main@_bb3_0)
                    (= main@%.16.i_1 main@%.16.i_0))
                (=> (and main@_bb4_0 main@_bb3_0)
                    (= main@%.14.i_1 main@%.14.i_0))
                (=> (and main@_bb4_0 main@_bb3_0)
                    (= main@%.12.i_1 main@%.12.i_0))
                main@_bb4_0)))
  (=> a!3
      (main@_bb4 main@%_0_0
                 main@%shadow.mem.0.0_0
                 main@%.12.i_1
                 main@%.16.i_1
                 main@%.14.i_1)))))
(rule (let ((a!1 (=> main@_bb5_0
               (= main@%_18_0 (+ main@%_0_0 (* 0 400000) (* main@%.12.i_0 4)))))
      (a!2 (=> main@_bb7_0
               (= main@%_24_0 (+ main@%_0_0 (* 0 400000) (* main@%.12.i_0 4))))))
(let ((a!3 (and (main@_bb4 main@%_0_0
                           main@%shadow.mem.0.0_0
                           main@%.12.i_0
                           main@%.16.i_0
                           main@%.14.i_0)
                true
                (= main@%_16_0 (< main@%.12.i_0 100000))
                (=> main@_bb5_0 (and main@_bb5_0 main@_bb4_0))
                (=> (and main@_bb5_0 main@_bb4_0) main@%_16_0)
                a!1
                (=> main@_bb5_0 (or (<= main@%_0_0 0) (> main@%_18_0 0)))
                (=> main@_bb5_0 (> main@%_0_0 0))
                (=> main@_bb5_0
                    (= main@%_19_0 (select main@%shadow.mem.0.0_0 main@%_18_0)))
                (=> main@_bb5_0 (= main@%_20_0 (< main@%_19_0 main@%.14.i_0)))
                (=> main@_bb6_0 (and main@_bb6_0 main@_bb5_0))
                (=> (and main@_bb6_0 main@_bb5_0) main@%_20_0)
                (=> main@_bb6_0 (= main@%_22_0 (> main@%_19_0 main@%.16.i_0)))
                (=> main@_bb7_0 (and main@_bb7_0 main@_bb6_0))
                (=> (and main@_bb7_0 main@_bb6_0) main@%_22_0)
                a!2
                (=> main@_bb7_0 (or (<= main@%_0_0 0) (> main@%_24_0 0)))
                (=> main@_bb7_0 (> main@%_0_0 0))
                (=> main@_bb7_0
                    (= main@%_25_0 (select main@%shadow.mem.0.0_0 main@%_24_0)))
                (=> |tuple(main@_bb6_0, main@_bb8_0)| main@_bb6_0)
                (=> |tuple(main@_bb5_0, main@_bb8_0)| main@_bb5_0)
                (=> main@_bb8_0
                    (or (and main@_bb8_0 main@_bb7_0)
                        |tuple(main@_bb6_0, main@_bb8_0)|
                        |tuple(main@_bb5_0, main@_bb8_0)|))
                (=> |tuple(main@_bb6_0, main@_bb8_0)| (not main@%_22_0))
                (=> |tuple(main@_bb5_0, main@_bb8_0)| (not main@%_20_0))
                (=> (and main@_bb8_0 main@_bb7_0) (= main@%.27.i_0 main@%_25_0))
                (=> (and main@_bb8_0 main@_bb7_0)
                    (= main@%.2.i_0 main@%.14.i_0))
                (=> |tuple(main@_bb6_0, main@_bb8_0)|
                    (= main@%.27.i_1 main@%.16.i_0))
                (=> |tuple(main@_bb6_0, main@_bb8_0)|
                    (= main@%.2.i_1 main@%.14.i_0))
                (=> |tuple(main@_bb5_0, main@_bb8_0)|
                    (= main@%.27.i_2 main@%.14.i_0))
                (=> |tuple(main@_bb5_0, main@_bb8_0)|
                    (= main@%.2.i_2 main@%_19_0))
                (=> (and main@_bb8_0 main@_bb7_0)
                    (= main@%.27.i_3 main@%.27.i_0))
                (=> (and main@_bb8_0 main@_bb7_0) (= main@%.2.i_3 main@%.2.i_0))
                (=> |tuple(main@_bb6_0, main@_bb8_0)|
                    (= main@%.27.i_3 main@%.27.i_1))
                (=> |tuple(main@_bb6_0, main@_bb8_0)|
                    (= main@%.2.i_3 main@%.2.i_1))
                (=> |tuple(main@_bb5_0, main@_bb8_0)|
                    (= main@%.27.i_3 main@%.27.i_2))
                (=> |tuple(main@_bb5_0, main@_bb8_0)|
                    (= main@%.2.i_3 main@%.2.i_2))
                (=> main@_bb8_0 (= main@%_27_0 (+ main@%.12.i_0 1)))
                (=> main@_bb4_1 (and main@_bb4_1 main@_bb8_0))
                (=> (and main@_bb4_1 main@_bb8_0)
                    (= main@%.16.i_1 main@%.27.i_3))
                (=> (and main@_bb4_1 main@_bb8_0)
                    (= main@%.14.i_1 main@%.2.i_3))
                (=> (and main@_bb4_1 main@_bb8_0) (= main@%.12.i_1 main@%_27_0))
                (=> (and main@_bb4_1 main@_bb8_0)
                    (= main@%.16.i_2 main@%.16.i_1))
                (=> (and main@_bb4_1 main@_bb8_0)
                    (= main@%.14.i_2 main@%.14.i_1))
                (=> (and main@_bb4_1 main@_bb8_0)
                    (= main@%.12.i_2 main@%.12.i_1))
                main@_bb4_1)))
  (=> a!3
      (main@_bb4 main@%_0_0
                 main@%shadow.mem.0.0_0
                 main@%.12.i_2
                 main@%.16.i_2
                 main@%.14.i_2)))))
(rule (=> (and (main@_bb4 main@%_0_0
                    main@%shadow.mem.0.0_0
                    main@%.12.i_0
                    main@%.16.i_0
                    main@%.14.i_0)
         true
         (= main@%_16_0 (< main@%.12.i_0 100000))
         (=> main@_bb9_0 (and main@_bb9_0 main@_bb4_0))
         (=> (and main@_bb9_0 main@_bb4_0) (not main@%_16_0))
         (=> main@_bb10_0 (and main@_bb10_0 main@_bb9_0))
         (=> (and main@_bb10_0 main@_bb9_0) (= main@%.0.i_0 0))
         (=> (and main@_bb10_0 main@_bb9_0) (= main@%.0.i_1 main@%.0.i_0))
         main@_bb10_0)
    (main@_bb10 main@%_0_0
                main@%shadow.mem.0.0_0
                main@%.16.i_0
                main@%.14.i_0
                main@%.0.i_1)))
(rule (let ((a!1 (=> main@_bb11_0
               (= main@%_32_0 (+ main@%_0_0 (* 0 400000) (* main@%.0.i_0 4))))))
(let ((a!2 (and (main@_bb10 main@%_0_0
                            main@%shadow.mem.0.0_0
                            main@%.16.i_0
                            main@%.14.i_0
                            main@%.0.i_0)
                true
                (= main@%_30_0 (< main@%.0.i_0 100000))
                (=> main@_bb11_0 (and main@_bb11_0 main@_bb10_0))
                (=> (and main@_bb11_0 main@_bb10_0) main@%_30_0)
                a!1
                (=> main@_bb11_0 (or (<= main@%_0_0 0) (> main@%_32_0 0)))
                (=> main@_bb11_0 (> main@%_0_0 0))
                (=> main@_bb11_0
                    (= main@%_33_0 (select main@%shadow.mem.0.0_0 main@%_32_0)))
                (=> main@_bb11_0 (= main@%_34_0 (<= main@%_33_0 main@%.14.i_0)))
                (=> main@_bb11_0 (= main@%_35_0 (ite main@%_34_0 1 0)))
                (=> main@postcall_0 (and main@postcall_0 main@_bb11_0))
                (=> (and main@postcall_0 main@_bb11_0) main@%_36_0)
                (__JVERIFIER_assert main@postcall_0 false false main@%_35_0)
                (=> main@postcall_0 (= main@%_37_0 (+ main@%.0.i_0 1)))
                (=> main@_bb10_1 (and main@_bb10_1 main@postcall_0))
                (=> (and main@_bb10_1 main@postcall_0)
                    (= main@%.0.i_1 main@%_37_0))
                (=> (and main@_bb10_1 main@postcall_0)
                    (= main@%.0.i_2 main@%.0.i_1))
                main@_bb10_1)))
  (=> a!2
      (main@_bb10 main@%_0_0
                  main@%shadow.mem.0.0_0
                  main@%.16.i_0
                  main@%.14.i_0
                  main@%.0.i_2)))))
(rule (=> (and (main@_bb10 main@%_0_0
                     main@%shadow.mem.0.0_0
                     main@%.16.i_0
                     main@%.14.i_0
                     main@%.0.i_0)
         true
         (= main@%_30_0 (< main@%.0.i_0 100000))
         (=> main@_bb12_0 (and main@_bb12_0 main@_bb10_0))
         (=> (and main@_bb12_0 main@_bb10_0) (not main@%_30_0))
         (=> main@_bb13_0 (and main@_bb13_0 main@_bb12_0))
         (=> (and main@_bb13_0 main@_bb12_0) (= main@%.1.i_0 0))
         (=> (and main@_bb13_0 main@_bb12_0) (= main@%.1.i_1 main@%.1.i_0))
         main@_bb13_0)
    (main@_bb13 main@%_0_0
                main@%shadow.mem.0.0_0
                main@%.16.i_0
                main@%.14.i_0
                main@%.1.i_1)))
(rule (let ((a!1 (=> main@_bb11_0
               (= main@%_32_0 (+ main@%_0_0 (* 0 400000) (* main@%.0.i_0 4))))))
(let ((a!2 (and (main@_bb10 main@%_0_0
                            main@%shadow.mem.0.0_0
                            main@%.16.i_0
                            main@%.14.i_0
                            main@%.0.i_0)
                true
                (= main@%_30_0 (< main@%.0.i_0 100000))
                (=> main@_bb11_0 (and main@_bb11_0 main@_bb10_0))
                (=> (and main@_bb11_0 main@_bb10_0) main@%_30_0)
                a!1
                (=> main@_bb11_0 (or (<= main@%_0_0 0) (> main@%_32_0 0)))
                (=> main@_bb11_0 (> main@%_0_0 0))
                (=> main@_bb11_0
                    (= main@%_33_0 (select main@%shadow.mem.0.0_0 main@%_32_0)))
                (=> main@_bb11_0 (= main@%_34_0 (<= main@%_33_0 main@%.14.i_0)))
                (=> main@_bb11_0 (= main@%_35_0 (ite main@%_34_0 1 0)))
                (=> main@precall_0 (and main@precall_0 main@_bb11_0))
                (=> (and main@precall_0 main@_bb11_0) (not main@%_36_0))
                (=> main@__JVERIFIER_assert_0
                    (and main@__JVERIFIER_assert_0 main@precall_0))
                (=> (and main@__JVERIFIER_assert_0 main@precall_0)
                    (= main@%.0_0 main@%_35_0))
                (=> (and main@__JVERIFIER_assert_0 main@precall_0)
                    (= main@%.0_1 main@%.0_0))
                (=> main@__JVERIFIER_assert_0 (= main@%_54_0 (= main@%.0_1 0)))
                (=> main@__JVERIFIER_assert_0 main@%_54_0)
                (=> main@_bb17_0 (and main@_bb17_0 main@__JVERIFIER_assert_0))
                (=> main@verifier.error_0
                    (and main@verifier.error_0 main@_bb17_0))
                (=> main@verifier.error.split_0
                    (and main@verifier.error.split_0 main@verifier.error_0))
                main@verifier.error.split_0)))
  (=> a!2 main@verifier.error.split))))
(rule (let ((a!1 (=> main@_bb14_0
               (= main@%_42_0 (+ main@%_0_0 (* 0 400000) (* main@%.1.i_0 4)))))
      (a!2 (=> main@_bb15_0
               (= main@%_46_0 (+ main@%_0_0 (* 0 400000) (* main@%.1.i_0 4))))))
(let ((a!3 (and (main@_bb13 main@%_0_0
                            main@%shadow.mem.0.0_0
                            main@%.16.i_0
                            main@%.14.i_0
                            main@%.1.i_0)
                true
                (= main@%_40_0 (< main@%.1.i_0 100000))
                main@%_40_0
                (=> main@_bb14_0 (and main@_bb14_0 main@_bb13_0))
                a!1
                (=> main@_bb14_0 (or (<= main@%_0_0 0) (> main@%_42_0 0)))
                (=> main@_bb14_0 (> main@%_0_0 0))
                (=> main@_bb14_0
                    (= main@%_43_0 (select main@%shadow.mem.0.0_0 main@%_42_0)))
                (=> main@_bb14_0 (= main@%_44_0 (> main@%_43_0 main@%.16.i_0)))
                (=> main@_bb15_0 (and main@_bb15_0 main@_bb14_0))
                (=> (and main@_bb15_0 main@_bb14_0) main@%_44_0)
                a!2
                (=> main@_bb15_0 (or (<= main@%_0_0 0) (> main@%_46_0 0)))
                (=> main@_bb15_0 (> main@%_0_0 0))
                (=> main@_bb15_0
                    (= main@%_47_0 (select main@%shadow.mem.0.0_0 main@%_46_0)))
                (=> main@_bb15_0 (= main@%_48_0 (= main@%_47_0 main@%.14.i_0)))
                (=> |tuple(main@_bb14_0, main@_bb16_0)| main@_bb14_0)
                (=> main@_bb16_0
                    (or (and main@_bb16_0 main@_bb15_0)
                        |tuple(main@_bb14_0, main@_bb16_0)|))
                (=> |tuple(main@_bb14_0, main@_bb16_0)| (not main@%_44_0))
                (=> (and main@_bb16_0 main@_bb15_0) (= main@%_50_0 main@%_48_0))
                (=> |tuple(main@_bb14_0, main@_bb16_0)| (= main@%_50_1 true))
                (=> (and main@_bb16_0 main@_bb15_0) (= main@%_50_2 main@%_50_0))
                (=> |tuple(main@_bb14_0, main@_bb16_0)|
                    (= main@%_50_2 main@%_50_1))
                (=> main@_bb16_0 (= main@%_51_0 (ite main@%_50_2 1 0)))
                (=> main@postcall1_0 (and main@postcall1_0 main@_bb16_0))
                (=> (and main@postcall1_0 main@_bb16_0) main@%_52_0)
                (__JVERIFIER_assert main@postcall1_0 false false main@%_51_0)
                (=> main@postcall1_0 (= main@%_53_0 (+ main@%.1.i_0 1)))
                (=> main@_bb13_1 (and main@_bb13_1 main@postcall1_0))
                (=> (and main@_bb13_1 main@postcall1_0)
                    (= main@%.1.i_1 main@%_53_0))
                (=> (and main@_bb13_1 main@postcall1_0)
                    (= main@%.1.i_2 main@%.1.i_1))
                main@_bb13_1)))
  (=> a!3
      (main@_bb13 main@%_0_0
                  main@%shadow.mem.0.0_0
                  main@%.16.i_0
                  main@%.14.i_0
                  main@%.1.i_2)))))
(rule (let ((a!1 (=> main@_bb14_0
               (= main@%_42_0 (+ main@%_0_0 (* 0 400000) (* main@%.1.i_0 4)))))
      (a!2 (=> main@_bb15_0
               (= main@%_46_0 (+ main@%_0_0 (* 0 400000) (* main@%.1.i_0 4))))))
(let ((a!3 (and (main@_bb13 main@%_0_0
                            main@%shadow.mem.0.0_0
                            main@%.16.i_0
                            main@%.14.i_0
                            main@%.1.i_0)
                true
                (= main@%_40_0 (< main@%.1.i_0 100000))
                main@%_40_0
                (=> main@_bb14_0 (and main@_bb14_0 main@_bb13_0))
                a!1
                (=> main@_bb14_0 (or (<= main@%_0_0 0) (> main@%_42_0 0)))
                (=> main@_bb14_0 (> main@%_0_0 0))
                (=> main@_bb14_0
                    (= main@%_43_0 (select main@%shadow.mem.0.0_0 main@%_42_0)))
                (=> main@_bb14_0 (= main@%_44_0 (> main@%_43_0 main@%.16.i_0)))
                (=> main@_bb15_0 (and main@_bb15_0 main@_bb14_0))
                (=> (and main@_bb15_0 main@_bb14_0) main@%_44_0)
                a!2
                (=> main@_bb15_0 (or (<= main@%_0_0 0) (> main@%_46_0 0)))
                (=> main@_bb15_0 (> main@%_0_0 0))
                (=> main@_bb15_0
                    (= main@%_47_0 (select main@%shadow.mem.0.0_0 main@%_46_0)))
                (=> main@_bb15_0 (= main@%_48_0 (= main@%_47_0 main@%.14.i_0)))
                (=> |tuple(main@_bb14_0, main@_bb16_0)| main@_bb14_0)
                (=> main@_bb16_0
                    (or (and main@_bb16_0 main@_bb15_0)
                        |tuple(main@_bb14_0, main@_bb16_0)|))
                (=> |tuple(main@_bb14_0, main@_bb16_0)| (not main@%_44_0))
                (=> (and main@_bb16_0 main@_bb15_0) (= main@%_50_0 main@%_48_0))
                (=> |tuple(main@_bb14_0, main@_bb16_0)| (= main@%_50_1 true))
                (=> (and main@_bb16_0 main@_bb15_0) (= main@%_50_2 main@%_50_0))
                (=> |tuple(main@_bb14_0, main@_bb16_0)|
                    (= main@%_50_2 main@%_50_1))
                (=> main@_bb16_0 (= main@%_51_0 (ite main@%_50_2 1 0)))
                (=> main@precall2_0 (and main@precall2_0 main@_bb16_0))
                (=> (and main@precall2_0 main@_bb16_0) (not main@%_52_0))
                (=> main@__JVERIFIER_assert_0
                    (and main@__JVERIFIER_assert_0 main@precall2_0))
                (=> (and main@__JVERIFIER_assert_0 main@precall2_0)
                    (= main@%.0_0 main@%_51_0))
                (=> (and main@__JVERIFIER_assert_0 main@precall2_0)
                    (= main@%.0_1 main@%.0_0))
                (=> main@__JVERIFIER_assert_0 (= main@%_54_0 (= main@%.0_1 0)))
                (=> main@__JVERIFIER_assert_0 main@%_54_0)
                (=> main@_bb17_0 (and main@_bb17_0 main@__JVERIFIER_assert_0))
                (=> main@verifier.error_0
                    (and main@verifier.error_0 main@_bb17_0))
                (=> main@verifier.error.split_0
                    (and main@verifier.error.split_0 main@verifier.error_0))
                main@verifier.error.split_0)))
  (=> a!3 main@verifier.error.split))))
(query main@verifier.error.split)


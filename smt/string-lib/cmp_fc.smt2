(set-info :original "/var/folders/xz/sgqbj6yn0yv_1mpy6_jqympm0000gn/T/sea-VVljVF/cmp_fc.pp.ms.o.bc")
(set-info :authors "SeaHorn v.0.1.0-rc3")
(declare-rel verifier.error (Bool Bool Bool ))
(declare-rel main@entry (Int Int ))
(declare-rel main@_bb (Int Int Int (Array Int Int) Int (Array Int Int) ))
(declare-rel main@_bb7 (Int Int Int Int (Array Int Int) (Array Int Int) Int ))
(declare-rel main@_bb9 (Int Int (Array Int Int) (Array Int Int) Int ))
(declare-rel main@_bb17 (Int Int Int (Array Int Int) (Array Int Int) ))
(declare-rel main@verifier.error.split ())
(declare-var main@%_7_0 Int )
(declare-var main@%_8_0 Int )
(declare-var main@%_9_0 Int )
(declare-var main@%_18_0 Int )
(declare-var main@%_19_0 Int )
(declare-var main@%_20_0 Int )
(declare-var main@%_66_0 Int )
(declare-var main@%_67_0 Int )
(declare-var main@%_68_0 Int )
(declare-var main@%_69_0 Int )
(declare-var main@%_70_0 Bool )
(declare-var main@%_71_0 Int )
(declare-var main@%_72_0 Bool )
(declare-var main@%_62_0 Int )
(declare-var main@%_63_0 Int )
(declare-var main@%_64_0 Bool )
(declare-var main@%_59_0 Bool )
(declare-var main@%_54_0 Int )
(declare-var main@%_55_0 Int )
(declare-var main@%_56_0 Int )
(declare-var main@%_57_0 Int )
(declare-var main@%_58_0 Bool )
(declare-var main@%_50_0 Int )
(declare-var main@%_51_0 Int )
(declare-var main@%_52_0 Bool )
(declare-var main@%_46_0 Int )
(declare-var main@%_47_0 Int )
(declare-var main@%_48_0 Bool )
(declare-var main@%_40_0 Int )
(declare-var main@%_41_0 Int )
(declare-var main@%_42_0 Int )
(declare-var main@%_43_0 Int )
(declare-var main@%_44_0 Bool )
(declare-var main@%.0.i3.i_4 Int )
(declare-var main@%_33_0 Int )
(declare-var main@%_34_0 Int )
(declare-var main@%_35_0 Int )
(declare-var main@%_36_0 Int )
(declare-var main@%_37_0 Bool )
(declare-var main@%_29_0 Int )
(declare-var main@%_30_0 Int )
(declare-var main@%_31_0 Bool )
(declare-var main@%_25_0 Int )
(declare-var main@%_26_0 Int )
(declare-var main@%_27_0 Bool )
(declare-var main@%_22_0 Int )
(declare-var main@%_23_0 Int )
(declare-var main@%_15_0 Int )
(declare-var main@%_16_0 Bool )
(declare-var main@%_11_0 Int )
(declare-var main@%_12_0 Int )
(declare-var main@%_4_0 Int )
(declare-var main@%_5_0 Bool )
(declare-var main@%_0_0 Int )
(declare-var @__VERIFIER_nondet_int_0 Int )
(declare-var @__VERIFIER_nondet_char_0 Int )
(declare-var main@entry_0 Bool )
(declare-var main@%sm4_0 (Array Int Int) )
(declare-var main@%sm5_0 (Array Int Int) )
(declare-var main@%_1_0 Int )
(declare-var main@%_2_0 Int )
(declare-var main@_bb_0 Bool )
(declare-var main@%shadow.mem.0.0_0 (Array Int Int) )
(declare-var main@%.0.i.i_0 Int )
(declare-var main@%shadow.mem.0.0_1 (Array Int Int) )
(declare-var main@%.0.i.i_1 Int )
(declare-var main@_bb6_0 Bool )
(declare-var main@%sm_0 (Array Int Int) )
(declare-var main@%_10_0 Int )
(declare-var main@_bb_1 Bool )
(declare-var main@%shadow.mem.0.0_2 (Array Int Int) )
(declare-var main@%.0.i.i_2 Int )
(declare-var main@init.exit.i_0 Bool )
(declare-var main@%sm1_0 (Array Int Int) )
(declare-var main@%_13_0 Int )
(declare-var main@_bb7_0 Bool )
(declare-var main@%shadow.mem.1.0_0 (Array Int Int) )
(declare-var main@%.0.i1.i_0 Int )
(declare-var main@%shadow.mem.1.0_1 (Array Int Int) )
(declare-var main@%.0.i1.i_1 Int )
(declare-var main@_bb8_0 Bool )
(declare-var main@%sm2_0 (Array Int Int) )
(declare-var main@%_21_0 Int )
(declare-var main@_bb7_1 Bool )
(declare-var main@%shadow.mem.1.0_2 (Array Int Int) )
(declare-var main@%.0.i1.i_2 Int )
(declare-var main@init.exit2.i_0 Bool )
(declare-var main@%sm3_0 (Array Int Int) )
(declare-var main@_bb9_0 Bool )
(declare-var main@%.01.i.i_0 Int )
(declare-var main@%.01.i.i_1 Int )
(declare-var main@_bb10_0 Bool )
(declare-var main@_bb11_0 Bool )
(declare-var main@_bb12_0 Bool )
(declare-var main@%_39_0 Int )
(declare-var main@_bb9_1 Bool )
(declare-var main@%.01.i.i_2 Int )
(declare-var main@.critedge.i.i_0 Bool )
(declare-var |tuple(main@_bb11_0, main@.critedge.i.i_0)| Bool )
(declare-var |tuple(main@_bb10_0, main@.critedge.i.i_0)| Bool )
(declare-var |tuple(main@_bb9_0, main@.critedge.i.i_0)| Bool )
(declare-var main@_bb13_0 Bool )
(declare-var main@_bb14_0 Bool )
(declare-var main@_bb15_0 Bool )
(declare-var main@%..i.i_0 Int )
(declare-var main@cmp.exit.i_0 Bool )
(declare-var |tuple(main@_bb14_0, main@cmp.exit.i_0)| Bool )
(declare-var |tuple(main@_bb13_0, main@cmp.exit.i_0)| Bool )
(declare-var |tuple(main@.critedge.i.i_0, main@cmp.exit.i_0)| Bool )
(declare-var main@%.0.i3.i_0 Int )
(declare-var main@%.0.i3.i_1 Int )
(declare-var main@%.0.i3.i_2 Int )
(declare-var main@%.0.i3.i_3 Int )
(declare-var main@_bb16_0 Bool )
(declare-var main@_bb17_0 Bool )
(declare-var main@%.0.i_0 Int )
(declare-var main@%.0.i_1 Int )
(declare-var main@_bb18_0 Bool )
(declare-var main@__JVERIFIER_assert.exit.i_0 Bool )
(declare-var main@%_74_0 Int )
(declare-var main@_bb17_1 Bool )
(declare-var main@%.0.i_2 Int )
(declare-var main@_bb19_0 Bool )
(declare-var main@verifier.error_0 Bool )
(declare-var main@verifier.error.split_0 Bool )
(rule (verifier.error false false false))
(rule (verifier.error false true true))
(rule (verifier.error true false true))
(rule (verifier.error true true true))
(rule (main@entry @__VERIFIER_nondet_char_0 @__VERIFIER_nondet_int_0))
(rule (=> (and (main@entry @__VERIFIER_nondet_char_0 @__VERIFIER_nondet_int_0)
         true
         (= main@%_0_0 @__VERIFIER_nondet_int_0)
         (> main@%_2_0 0)
         (=> main@_bb_0 (and main@_bb_0 main@entry_0))
         (=> (and main@_bb_0 main@entry_0)
             (= main@%shadow.mem.0.0_0 main@%sm5_0))
         (=> (and main@_bb_0 main@entry_0) (= main@%.0.i.i_0 0))
         (=> (and main@_bb_0 main@entry_0)
             (= main@%shadow.mem.0.0_1 main@%shadow.mem.0.0_0))
         (=> (and main@_bb_0 main@entry_0) (= main@%.0.i.i_1 main@%.0.i.i_0))
         main@_bb_0)
    (main@_bb @__VERIFIER_nondet_char_0
              main@%_2_0
              main@%.0.i.i_1
              main@%shadow.mem.0.0_1
              main@%_1_0
              main@%sm4_0)))
(rule (let ((a!1 (=> main@_bb6_0 (= main@%_9_0 (+ main@%_2_0 (* main@%.0.i.i_0 1))))))
(let ((a!2 (and (main@_bb @__VERIFIER_nondet_char_0
                          main@%_2_0
                          main@%.0.i.i_0
                          main@%shadow.mem.0.0_0
                          main@%_1_0
                          main@%sm4_0)
                true
                (= main@%_4_0 (+ main@%_1_0 (- 1)))
                (= main@%_5_0 (< main@%.0.i.i_0 main@%_4_0))
                (=> main@_bb6_0 (and main@_bb6_0 main@_bb_0))
                (=> (and main@_bb6_0 main@_bb_0) main@%_5_0)
                (=> main@_bb6_0 (= main@%_7_0 @__VERIFIER_nondet_char_0))
                a!1
                (=> main@_bb6_0 (or (<= main@%_2_0 0) (> main@%_9_0 0)))
                (=> main@_bb6_0 (> main@%_2_0 0))
                (=> main@_bb6_0
                    (= main@%sm_0
                       (store main@%shadow.mem.0.0_0 main@%_9_0 main@%_8_0)))
                (=> main@_bb6_0 (= main@%_10_0 (+ main@%.0.i.i_0 1)))
                (=> main@_bb_1 (and main@_bb_1 main@_bb6_0))
                (=> (and main@_bb_1 main@_bb6_0)
                    (= main@%shadow.mem.0.0_1 main@%sm_0))
                (=> (and main@_bb_1 main@_bb6_0) (= main@%.0.i.i_1 main@%_10_0))
                (=> (and main@_bb_1 main@_bb6_0)
                    (= main@%shadow.mem.0.0_2 main@%shadow.mem.0.0_1))
                (=> (and main@_bb_1 main@_bb6_0)
                    (= main@%.0.i.i_2 main@%.0.i.i_1))
                main@_bb_1)))
  (=> a!2
      (main@_bb @__VERIFIER_nondet_char_0
                main@%_2_0
                main@%.0.i.i_2
                main@%shadow.mem.0.0_2
                main@%_1_0
                main@%sm4_0)))))
(rule (let ((a!1 (=> main@init.exit.i_0
               (= main@%_12_0 (+ main@%_2_0 (* main@%_11_0 1))))))
(let ((a!2 (and (main@_bb @__VERIFIER_nondet_char_0
                          main@%_2_0
                          main@%.0.i.i_0
                          main@%shadow.mem.0.0_0
                          main@%_1_0
                          main@%sm4_0)
                true
                (= main@%_4_0 (+ main@%_1_0 (- 1)))
                (= main@%_5_0 (< main@%.0.i.i_0 main@%_4_0))
                (=> main@init.exit.i_0 (and main@init.exit.i_0 main@_bb_0))
                (=> (and main@init.exit.i_0 main@_bb_0) (not main@%_5_0))
                (=> main@init.exit.i_0 (= main@%_11_0 (+ main@%_1_0 (- 1))))
                a!1
                (=> main@init.exit.i_0 (or (<= main@%_2_0 0) (> main@%_12_0 0)))
                (=> main@init.exit.i_0 (> main@%_2_0 0))
                (=> main@init.exit.i_0
                    (= main@%sm1_0 (store main@%shadow.mem.0.0_0 main@%_12_0 0)))
                (=> main@init.exit.i_0 (> main@%_13_0 0))
                (=> main@_bb7_0 (and main@_bb7_0 main@init.exit.i_0))
                (=> (and main@_bb7_0 main@init.exit.i_0)
                    (= main@%shadow.mem.1.0_0 main@%sm4_0))
                (=> (and main@_bb7_0 main@init.exit.i_0) (= main@%.0.i1.i_0 0))
                (=> (and main@_bb7_0 main@init.exit.i_0)
                    (= main@%shadow.mem.1.0_1 main@%shadow.mem.1.0_0))
                (=> (and main@_bb7_0 main@init.exit.i_0)
                    (= main@%.0.i1.i_1 main@%.0.i1.i_0))
                main@_bb7_0)))
  (=> a!2
      (main@_bb7 @__VERIFIER_nondet_char_0
                 main@%_2_0
                 main@%_13_0
                 main@%.0.i1.i_1
                 main@%shadow.mem.1.0_1
                 main@%sm1_0
                 main@%_1_0)))))
(rule (let ((a!1 (=> main@_bb8_0
               (= main@%_20_0 (+ main@%_13_0 (* main@%.0.i1.i_0 1))))))
(let ((a!2 (and (main@_bb7 @__VERIFIER_nondet_char_0
                           main@%_2_0
                           main@%_13_0
                           main@%.0.i1.i_0
                           main@%shadow.mem.1.0_0
                           main@%sm1_0
                           main@%_1_0)
                true
                (= main@%_15_0 (+ main@%_1_0 (- 1)))
                (= main@%_16_0 (< main@%.0.i1.i_0 main@%_15_0))
                (=> main@_bb8_0 (and main@_bb8_0 main@_bb7_0))
                (=> (and main@_bb8_0 main@_bb7_0) main@%_16_0)
                (=> main@_bb8_0 (= main@%_18_0 @__VERIFIER_nondet_char_0))
                a!1
                (=> main@_bb8_0 (or (<= main@%_13_0 0) (> main@%_20_0 0)))
                (=> main@_bb8_0 (> main@%_13_0 0))
                (=> main@_bb8_0
                    (= main@%sm2_0
                       (store main@%shadow.mem.1.0_0 main@%_20_0 main@%_19_0)))
                (=> main@_bb8_0 (= main@%_21_0 (+ main@%.0.i1.i_0 1)))
                (=> main@_bb7_1 (and main@_bb7_1 main@_bb8_0))
                (=> (and main@_bb7_1 main@_bb8_0)
                    (= main@%shadow.mem.1.0_1 main@%sm2_0))
                (=> (and main@_bb7_1 main@_bb8_0)
                    (= main@%.0.i1.i_1 main@%_21_0))
                (=> (and main@_bb7_1 main@_bb8_0)
                    (= main@%shadow.mem.1.0_2 main@%shadow.mem.1.0_1))
                (=> (and main@_bb7_1 main@_bb8_0)
                    (= main@%.0.i1.i_2 main@%.0.i1.i_1))
                main@_bb7_1)))
  (=> a!2
      (main@_bb7 @__VERIFIER_nondet_char_0
                 main@%_2_0
                 main@%_13_0
                 main@%.0.i1.i_2
                 main@%shadow.mem.1.0_2
                 main@%sm1_0
                 main@%_1_0)))))
(rule (let ((a!1 (=> main@init.exit2.i_0
               (= main@%_23_0 (+ main@%_13_0 (* main@%_22_0 1))))))
(let ((a!2 (and (main@_bb7 @__VERIFIER_nondet_char_0
                           main@%_2_0
                           main@%_13_0
                           main@%.0.i1.i_0
                           main@%shadow.mem.1.0_0
                           main@%sm1_0
                           main@%_1_0)
                true
                (= main@%_15_0 (+ main@%_1_0 (- 1)))
                (= main@%_16_0 (< main@%.0.i1.i_0 main@%_15_0))
                (=> main@init.exit2.i_0 (and main@init.exit2.i_0 main@_bb7_0))
                (=> (and main@init.exit2.i_0 main@_bb7_0) (not main@%_16_0))
                (=> main@init.exit2.i_0 (= main@%_22_0 (+ main@%_1_0 (- 1))))
                a!1
                (=> main@init.exit2.i_0
                    (or (<= main@%_13_0 0) (> main@%_23_0 0)))
                (=> main@init.exit2.i_0 (> main@%_13_0 0))
                (=> main@init.exit2.i_0
                    (= main@%sm3_0 (store main@%shadow.mem.1.0_0 main@%_23_0 0)))
                (=> main@_bb9_0 (and main@_bb9_0 main@init.exit2.i_0))
                (=> (and main@_bb9_0 main@init.exit2.i_0) (= main@%.01.i.i_0 0))
                (=> (and main@_bb9_0 main@init.exit2.i_0)
                    (= main@%.01.i.i_1 main@%.01.i.i_0))
                main@_bb9_0)))
  (=> a!2
      (main@_bb9 main@%_2_0 main@%_13_0 main@%sm1_0 main@%sm3_0 main@%.01.i.i_1)))))
(rule (let ((a!1 (=> main@_bb10_0
               (= main@%_29_0 (+ main@%_13_0 (* main@%.01.i.i_0 1)))))
      (a!2 (=> main@_bb11_0
               (= main@%_33_0 (+ main@%_2_0 (* main@%.01.i.i_0 1)))))
      (a!3 (=> main@_bb11_0
               (= main@%_35_0 (+ main@%_13_0 (* main@%.01.i.i_0 1))))))
(let ((a!4 (and (main@_bb9 main@%_2_0
                           main@%_13_0
                           main@%sm1_0
                           main@%sm3_0
                           main@%.01.i.i_0)
                true
                (= main@%_25_0 (+ main@%_2_0 (* main@%.01.i.i_0 1)))
                (or (<= main@%_2_0 0) (> main@%_25_0 0))
                (> main@%_2_0 0)
                (= main@%_26_0 (select main@%sm1_0 main@%_25_0))
                (= main@%_27_0 (= main@%_26_0 0))
                (=> main@_bb10_0 (and main@_bb10_0 main@_bb9_0))
                (=> (and main@_bb10_0 main@_bb9_0) (not main@%_27_0))
                a!1
                (=> main@_bb10_0 (or (<= main@%_13_0 0) (> main@%_29_0 0)))
                (=> main@_bb10_0 (> main@%_13_0 0))
                (=> main@_bb10_0
                    (= main@%_30_0 (select main@%sm3_0 main@%_29_0)))
                (=> main@_bb10_0 (= main@%_31_0 (= main@%_30_0 0)))
                (=> main@_bb11_0 (and main@_bb11_0 main@_bb10_0))
                (=> (and main@_bb11_0 main@_bb10_0) (not main@%_31_0))
                a!2
                (=> main@_bb11_0 (or (<= main@%_2_0 0) (> main@%_33_0 0)))
                (=> main@_bb11_0 (> main@%_2_0 0))
                (=> main@_bb11_0
                    (= main@%_34_0 (select main@%sm1_0 main@%_33_0)))
                a!3
                (=> main@_bb11_0 (or (<= main@%_13_0 0) (> main@%_35_0 0)))
                (=> main@_bb11_0 (> main@%_13_0 0))
                (=> main@_bb11_0
                    (= main@%_36_0 (select main@%sm3_0 main@%_35_0)))
                (=> main@_bb11_0 (= main@%_37_0 (= main@%_34_0 main@%_36_0)))
                (=> main@_bb12_0 (and main@_bb12_0 main@_bb11_0))
                (=> (and main@_bb12_0 main@_bb11_0) main@%_37_0)
                (=> main@_bb12_0 (= main@%_39_0 (+ main@%.01.i.i_0 1)))
                (=> main@_bb9_1 (and main@_bb9_1 main@_bb12_0))
                (=> (and main@_bb9_1 main@_bb12_0)
                    (= main@%.01.i.i_1 main@%_39_0))
                (=> (and main@_bb9_1 main@_bb12_0)
                    (= main@%.01.i.i_2 main@%.01.i.i_1))
                main@_bb9_1)))
  (=> a!4
      (main@_bb9 main@%_2_0 main@%_13_0 main@%sm1_0 main@%sm3_0 main@%.01.i.i_2)))))
(rule (let ((a!1 (=> main@_bb10_0
               (= main@%_29_0 (+ main@%_13_0 (* main@%.01.i.i_0 1)))))
      (a!2 (=> main@_bb11_0
               (= main@%_33_0 (+ main@%_2_0 (* main@%.01.i.i_0 1)))))
      (a!3 (=> main@_bb11_0
               (= main@%_35_0 (+ main@%_13_0 (* main@%.01.i.i_0 1)))))
      (a!4 (=> main@.critedge.i.i_0
               (= main@%_40_0 (+ main@%_2_0 (* main@%.01.i.i_0 1)))))
      (a!5 (=> main@.critedge.i.i_0
               (= main@%_42_0 (+ main@%_13_0 (* main@%.01.i.i_0 1)))))
      (a!6 (=> main@_bb13_0
               (= main@%_46_0 (+ main@%_2_0 (* main@%.01.i.i_0 1)))))
      (a!7 (=> main@_bb14_0
               (= main@%_50_0 (+ main@%_13_0 (* main@%.01.i.i_0 1)))))
      (a!8 (=> main@_bb15_0
               (= main@%_54_0 (+ main@%_2_0 (* main@%.01.i.i_0 1)))))
      (a!9 (=> main@_bb15_0
               (= main@%_56_0 (+ main@%_13_0 (* main@%.01.i.i_0 1))))))
(let ((a!10 (and (main@_bb9 main@%_2_0
                            main@%_13_0
                            main@%sm1_0
                            main@%sm3_0
                            main@%.01.i.i_0)
                 true
                 (= main@%_25_0 (+ main@%_2_0 (* main@%.01.i.i_0 1)))
                 (or (<= main@%_2_0 0) (> main@%_25_0 0))
                 (> main@%_2_0 0)
                 (= main@%_26_0 (select main@%sm1_0 main@%_25_0))
                 (= main@%_27_0 (= main@%_26_0 0))
                 (=> main@_bb10_0 (and main@_bb10_0 main@_bb9_0))
                 (=> (and main@_bb10_0 main@_bb9_0) (not main@%_27_0))
                 a!1
                 (=> main@_bb10_0 (or (<= main@%_13_0 0) (> main@%_29_0 0)))
                 (=> main@_bb10_0 (> main@%_13_0 0))
                 (=> main@_bb10_0
                     (= main@%_30_0 (select main@%sm3_0 main@%_29_0)))
                 (=> main@_bb10_0 (= main@%_31_0 (= main@%_30_0 0)))
                 (=> main@_bb11_0 (and main@_bb11_0 main@_bb10_0))
                 (=> (and main@_bb11_0 main@_bb10_0) (not main@%_31_0))
                 a!2
                 (=> main@_bb11_0 (or (<= main@%_2_0 0) (> main@%_33_0 0)))
                 (=> main@_bb11_0 (> main@%_2_0 0))
                 (=> main@_bb11_0
                     (= main@%_34_0 (select main@%sm1_0 main@%_33_0)))
                 a!3
                 (=> main@_bb11_0 (or (<= main@%_13_0 0) (> main@%_35_0 0)))
                 (=> main@_bb11_0 (> main@%_13_0 0))
                 (=> main@_bb11_0
                     (= main@%_36_0 (select main@%sm3_0 main@%_35_0)))
                 (=> main@_bb11_0 (= main@%_37_0 (= main@%_34_0 main@%_36_0)))
                 (=> |tuple(main@_bb11_0, main@.critedge.i.i_0)| main@_bb11_0)
                 (=> |tuple(main@_bb10_0, main@.critedge.i.i_0)| main@_bb10_0)
                 (=> |tuple(main@_bb9_0, main@.critedge.i.i_0)| main@_bb9_0)
                 (=> main@.critedge.i.i_0
                     (or |tuple(main@_bb11_0, main@.critedge.i.i_0)|
                         |tuple(main@_bb10_0, main@.critedge.i.i_0)|
                         |tuple(main@_bb9_0, main@.critedge.i.i_0)|))
                 (=> |tuple(main@_bb11_0, main@.critedge.i.i_0)|
                     (not main@%_37_0))
                 (=> |tuple(main@_bb10_0, main@.critedge.i.i_0)| main@%_31_0)
                 (=> |tuple(main@_bb9_0, main@.critedge.i.i_0)| main@%_27_0)
                 a!4
                 (=> main@.critedge.i.i_0
                     (or (<= main@%_2_0 0) (> main@%_40_0 0)))
                 (=> main@.critedge.i.i_0 (> main@%_2_0 0))
                 (=> main@.critedge.i.i_0
                     (= main@%_41_0 (select main@%sm1_0 main@%_40_0)))
                 a!5
                 (=> main@.critedge.i.i_0
                     (or (<= main@%_13_0 0) (> main@%_42_0 0)))
                 (=> main@.critedge.i.i_0 (> main@%_13_0 0))
                 (=> main@.critedge.i.i_0
                     (= main@%_43_0 (select main@%sm3_0 main@%_42_0)))
                 (=> main@.critedge.i.i_0
                     (= main@%_44_0 (= main@%_41_0 main@%_43_0)))
                 (=> main@_bb13_0 (and main@_bb13_0 main@.critedge.i.i_0))
                 (=> (and main@_bb13_0 main@.critedge.i.i_0) (not main@%_44_0))
                 a!6
                 (=> main@_bb13_0 (or (<= main@%_2_0 0) (> main@%_46_0 0)))
                 (=> main@_bb13_0 (> main@%_2_0 0))
                 (=> main@_bb13_0
                     (= main@%_47_0 (select main@%sm1_0 main@%_46_0)))
                 (=> main@_bb13_0 (= main@%_48_0 (= main@%_47_0 0)))
                 (=> main@_bb14_0 (and main@_bb14_0 main@_bb13_0))
                 (=> (and main@_bb14_0 main@_bb13_0) (not main@%_48_0))
                 a!7
                 (=> main@_bb14_0 (or (<= main@%_13_0 0) (> main@%_50_0 0)))
                 (=> main@_bb14_0 (> main@%_13_0 0))
                 (=> main@_bb14_0
                     (= main@%_51_0 (select main@%sm3_0 main@%_50_0)))
                 (=> main@_bb14_0 (= main@%_52_0 (= main@%_51_0 0)))
                 (=> main@_bb15_0 (and main@_bb15_0 main@_bb14_0))
                 (=> (and main@_bb15_0 main@_bb14_0) (not main@%_52_0))
                 a!8
                 (=> main@_bb15_0 (or (<= main@%_2_0 0) (> main@%_54_0 0)))
                 (=> main@_bb15_0 (> main@%_2_0 0))
                 (=> main@_bb15_0
                     (= main@%_55_0 (select main@%sm1_0 main@%_54_0)))
                 a!9
                 (=> main@_bb15_0 (or (<= main@%_13_0 0) (> main@%_56_0 0)))
                 (=> main@_bb15_0 (> main@%_13_0 0))
                 (=> main@_bb15_0
                     (= main@%_57_0 (select main@%sm3_0 main@%_56_0)))
                 (=> main@_bb15_0 (= main@%_58_0 (< main@%_55_0 main@%_57_0)))
                 (=> main@_bb15_0 (= main@%..i.i_0 (ite main@%_58_0 (- 1) 1)))
                 (=> |tuple(main@_bb14_0, main@cmp.exit.i_0)| main@_bb14_0)
                 (=> |tuple(main@_bb13_0, main@cmp.exit.i_0)| main@_bb13_0)
                 (=> |tuple(main@.critedge.i.i_0, main@cmp.exit.i_0)|
                     main@.critedge.i.i_0)
                 (=> main@cmp.exit.i_0
                     (or (and main@cmp.exit.i_0 main@_bb15_0)
                         |tuple(main@_bb14_0, main@cmp.exit.i_0)|
                         |tuple(main@_bb13_0, main@cmp.exit.i_0)|
                         |tuple(main@.critedge.i.i_0, main@cmp.exit.i_0)|))
                 (=> |tuple(main@_bb14_0, main@cmp.exit.i_0)| main@%_52_0)
                 (=> |tuple(main@_bb13_0, main@cmp.exit.i_0)| main@%_48_0)
                 (=> |tuple(main@.critedge.i.i_0, main@cmp.exit.i_0)|
                     main@%_44_0)
                 (=> (and main@cmp.exit.i_0 main@_bb15_0)
                     (= main@%.0.i3.i_0 main@%..i.i_0))
                 (=> |tuple(main@_bb14_0, main@cmp.exit.i_0)|
                     (= main@%.0.i3.i_1 1))
                 (=> |tuple(main@_bb13_0, main@cmp.exit.i_0)|
                     (= main@%.0.i3.i_2 (- 1)))
                 (=> |tuple(main@.critedge.i.i_0, main@cmp.exit.i_0)|
                     (= main@%.0.i3.i_3 0))
                 (=> (and main@cmp.exit.i_0 main@_bb15_0)
                     (= main@%.0.i3.i_4 main@%.0.i3.i_0))
                 (=> |tuple(main@_bb14_0, main@cmp.exit.i_0)|
                     (= main@%.0.i3.i_4 main@%.0.i3.i_1))
                 (=> |tuple(main@_bb13_0, main@cmp.exit.i_0)|
                     (= main@%.0.i3.i_4 main@%.0.i3.i_2))
                 (=> |tuple(main@.critedge.i.i_0, main@cmp.exit.i_0)|
                     (= main@%.0.i3.i_4 main@%.0.i3.i_3))
                 (=> main@cmp.exit.i_0 (= main@%_59_0 (= main@%.0.i3.i_4 0)))
                 (=> main@cmp.exit.i_0 main@%_59_0)
                 (=> main@_bb16_0 (and main@_bb16_0 main@cmp.exit.i_0))
                 (=> main@_bb17_0 (and main@_bb17_0 main@_bb16_0))
                 (=> (and main@_bb17_0 main@_bb16_0) (= main@%.0.i_0 0))
                 (=> (and main@_bb17_0 main@_bb16_0)
                     (= main@%.0.i_1 main@%.0.i_0))
                 main@_bb17_0)))
  (=> a!10
      (main@_bb17 main@%_2_0 main@%_13_0 main@%.0.i_1 main@%sm1_0 main@%sm3_0)))))
(rule (let ((a!1 (=> main@_bb18_0 (= main@%_66_0 (+ main@%_2_0 (* main@%.0.i_0 1)))))
      (a!2 (=> main@_bb18_0 (= main@%_68_0 (+ main@%_13_0 (* main@%.0.i_0 1))))))
(let ((a!3 (and (main@_bb17 main@%_2_0
                            main@%_13_0
                            main@%.0.i_0
                            main@%sm1_0
                            main@%sm3_0)
                true
                (= main@%_62_0 (+ main@%_2_0 (* main@%.0.i_0 1)))
                (or (<= main@%_2_0 0) (> main@%_62_0 0))
                (> main@%_2_0 0)
                (= main@%_63_0 (select main@%sm1_0 main@%_62_0))
                (= main@%_64_0 (= main@%_63_0 0))
                (not main@%_64_0)
                (=> main@_bb18_0 (and main@_bb18_0 main@_bb17_0))
                a!1
                (=> main@_bb18_0 (or (<= main@%_2_0 0) (> main@%_66_0 0)))
                (=> main@_bb18_0 (> main@%_2_0 0))
                (=> main@_bb18_0
                    (= main@%_67_0 (select main@%sm1_0 main@%_66_0)))
                a!2
                (=> main@_bb18_0 (or (<= main@%_13_0 0) (> main@%_68_0 0)))
                (=> main@_bb18_0 (> main@%_13_0 0))
                (=> main@_bb18_0
                    (= main@%_69_0 (select main@%sm3_0 main@%_68_0)))
                (=> main@_bb18_0 (= main@%_70_0 (= main@%_67_0 main@%_69_0)))
                (=> main@_bb18_0 (= main@%_71_0 (ite main@%_70_0 1 0)))
                (=> main@_bb18_0 (= main@%_72_0 (= main@%_71_0 0)))
                (=> main@__JVERIFIER_assert.exit.i_0
                    (and main@__JVERIFIER_assert.exit.i_0 main@_bb18_0))
                (=> (and main@__JVERIFIER_assert.exit.i_0 main@_bb18_0)
                    (not main@%_72_0))
                (=> main@__JVERIFIER_assert.exit.i_0
                    (= main@%_74_0 (+ main@%.0.i_0 1)))
                (=> main@_bb17_1
                    (and main@_bb17_1 main@__JVERIFIER_assert.exit.i_0))
                (=> (and main@_bb17_1 main@__JVERIFIER_assert.exit.i_0)
                    (= main@%.0.i_1 main@%_74_0))
                (=> (and main@_bb17_1 main@__JVERIFIER_assert.exit.i_0)
                    (= main@%.0.i_2 main@%.0.i_1))
                main@_bb17_1)))
  (=> a!3
      (main@_bb17 main@%_2_0 main@%_13_0 main@%.0.i_2 main@%sm1_0 main@%sm3_0)))))
(rule (let ((a!1 (=> main@_bb18_0 (= main@%_66_0 (+ main@%_2_0 (* main@%.0.i_0 1)))))
      (a!2 (=> main@_bb18_0 (= main@%_68_0 (+ main@%_13_0 (* main@%.0.i_0 1))))))
(let ((a!3 (and (main@_bb17 main@%_2_0
                            main@%_13_0
                            main@%.0.i_0
                            main@%sm1_0
                            main@%sm3_0)
                true
                (= main@%_62_0 (+ main@%_2_0 (* main@%.0.i_0 1)))
                (or (<= main@%_2_0 0) (> main@%_62_0 0))
                (> main@%_2_0 0)
                (= main@%_63_0 (select main@%sm1_0 main@%_62_0))
                (= main@%_64_0 (= main@%_63_0 0))
                (not main@%_64_0)
                (=> main@_bb18_0 (and main@_bb18_0 main@_bb17_0))
                a!1
                (=> main@_bb18_0 (or (<= main@%_2_0 0) (> main@%_66_0 0)))
                (=> main@_bb18_0 (> main@%_2_0 0))
                (=> main@_bb18_0
                    (= main@%_67_0 (select main@%sm1_0 main@%_66_0)))
                a!2
                (=> main@_bb18_0 (or (<= main@%_13_0 0) (> main@%_68_0 0)))
                (=> main@_bb18_0 (> main@%_13_0 0))
                (=> main@_bb18_0
                    (= main@%_69_0 (select main@%sm3_0 main@%_68_0)))
                (=> main@_bb18_0 (= main@%_70_0 (= main@%_67_0 main@%_69_0)))
                (=> main@_bb18_0 (= main@%_71_0 (ite main@%_70_0 1 0)))
                (=> main@_bb18_0 (= main@%_72_0 (= main@%_71_0 0)))
                (=> main@_bb19_0 (and main@_bb19_0 main@_bb18_0))
                (=> (and main@_bb19_0 main@_bb18_0) main@%_72_0)
                (=> main@verifier.error_0
                    (and main@verifier.error_0 main@_bb19_0))
                (=> main@verifier.error.split_0
                    (and main@verifier.error.split_0 main@verifier.error_0))
                main@verifier.error.split_0)))
  (=> a!3 main@verifier.error.split))))
(query main@verifier.error.split)


(set-info :original "/var/folders/xz/sgqbj6yn0yv_1mpy6_jqympm0000gn/T/sea-rmMpnd/partial_mod_count_limited_base.pp.ms.o.bc")
(set-info :authors "SeaHorn v.0.1.0-rc3")
(declare-rel verifier.error (Bool Bool Bool ))
(declare-rel __JVERIFIER_assert@_call (Int ))
(declare-rel __JVERIFIER_assert@_ret (Int ))
(declare-rel __JVERIFIER_assert (Bool Bool Bool Int ))
(declare-rel main@entry ())
(declare-rel main@_bb (Int Int Int (Array Int Int) Int ))
(declare-rel main@_bb7 (Int (Array Int Int) Int Int ))
(declare-rel main@verifier.error.split ())
(declare-var __JVERIFIER_assert@%_call1_0 Bool )
(declare-var __JVERIFIER_assert@arg.0_0 Int )
(declare-var __JVERIFIER_assert@_call_0 Bool )
(declare-var __JVERIFIER_assert@_ret_0 Bool )
(declare-var main@%_15_0 Int )
(declare-var main@%_16_0 Int )
(declare-var main@%_17_0 Bool )
(declare-var main@%_18_0 Int )
(declare-var main@%_12_0 Int )
(declare-var main@%_13_0 Int )
(declare-var main@%_7_0 Int )
(declare-var main@%_8_0 Int )
(declare-var main@%_9_0 Bool )
(declare-var main@%_10_0 Bool )
(declare-var main@%or.cond.i_0 Bool )
(declare-var main@%_32_0 Bool )
(declare-var main@%_24_0 Int )
(declare-var main@%_25_0 Int )
(declare-var main@%_26_0 Bool )
(declare-var main@%_27_0 Int )
(declare-var main@%_28_0 Bool )
(declare-var main@%_30_0 Bool )
(declare-var main@%_22_0 Bool )
(declare-var main@%.04.i_2 Int )
(declare-var main@%_5_0 Bool )
(declare-var main@%_1_0 Int )
(declare-var main@%_3_0 Bool )
(declare-var main@entry_0 Bool )
(declare-var main@%sm2_0 (Array Int Int) )
(declare-var main@%_0_0 Int )
(declare-var main@%_2_0 Int )
(declare-var main@_bb_0 Bool )
(declare-var main@%shadow.mem.0.0_0 (Array Int Int) )
(declare-var main@%.02.i_0 Int )
(declare-var main@%.0.i_0 Int )
(declare-var main@%shadow.mem.0.0_1 (Array Int Int) )
(declare-var main@%.02.i_1 Int )
(declare-var main@%.0.i_1 Int )
(declare-var main@_bb3_0 Bool )
(declare-var main@%sm_0 (Array Int Int) )
(declare-var main@_bb4_0 Bool )
(declare-var main@%sm1_0 (Array Int Int) )
(declare-var main@_bb5_0 Bool )
(declare-var |tuple(main@_bb3_0, main@_bb5_0)| Bool )
(declare-var main@%shadow.mem.0.1_0 (Array Int Int) )
(declare-var main@%shadow.mem.0.1_1 (Array Int Int) )
(declare-var main@%shadow.mem.0.1_2 (Array Int Int) )
(declare-var main@%..0.i_0 Int )
(declare-var main@%_19_0 Int )
(declare-var main@_bb_1 Bool )
(declare-var main@%shadow.mem.0.0_2 (Array Int Int) )
(declare-var main@%.02.i_2 Int )
(declare-var main@%.0.i_2 Int )
(declare-var main@_bb6_0 Bool )
(declare-var main@_bb7_0 Bool )
(declare-var main@%.04.i_0 Int )
(declare-var main@%.13.i_0 Int )
(declare-var main@%.04.i_1 Int )
(declare-var main@%.13.i_1 Int )
(declare-var main@_bb8_0 Bool )
(declare-var main@%..04.i_0 Int )
(declare-var main@%_29_0 Int )
(declare-var main@postcall_0 Bool )
(declare-var main@%_31_0 Int )
(declare-var main@_bb7_1 Bool )
(declare-var main@%.13.i_2 Int )
(declare-var main@precall_0 Bool )
(declare-var main@__JVERIFIER_assert_0 Bool )
(declare-var main@_bb9_0 Bool )
(declare-var main@verifier.error_0 Bool )
(declare-var main@verifier.error.split_0 Bool )
(rule (verifier.error false false false))
(rule (verifier.error false true true))
(rule (verifier.error true false true))
(rule (verifier.error true true true))
(rule (__JVERIFIER_assert true true true __JVERIFIER_assert@arg.0_0))
(rule (__JVERIFIER_assert false true true __JVERIFIER_assert@arg.0_0))
(rule (__JVERIFIER_assert false false false __JVERIFIER_assert@arg.0_0))
(rule (__JVERIFIER_assert@_call __JVERIFIER_assert@arg.0_0))
(rule (=> (and (__JVERIFIER_assert@_call __JVERIFIER_assert@arg.0_0)
         true
         (= __JVERIFIER_assert@%_call1_0 (= __JVERIFIER_assert@arg.0_0 0))
         (not __JVERIFIER_assert@%_call1_0)
         (=> __JVERIFIER_assert@_ret_0
             (and __JVERIFIER_assert@_ret_0 __JVERIFIER_assert@_call_0))
         __JVERIFIER_assert@_ret_0)
    (__JVERIFIER_assert@_ret __JVERIFIER_assert@arg.0_0)))
(rule (=> (__JVERIFIER_assert@_ret __JVERIFIER_assert@arg.0_0)
    (__JVERIFIER_assert true false false __JVERIFIER_assert@arg.0_0)))
(rule main@entry)
(rule (let ((a!1 (and main@entry
                true
                (> main@%_0_0 0)
                (= main@%_1_0 main@%_0_0)
                (= main@%_3_0 (not (= main@%_2_0 0)))
                main@%_3_0
                (=> main@_bb_0 (and main@_bb_0 main@entry_0))
                (=> (and main@_bb_0 main@entry_0)
                    (= main@%shadow.mem.0.0_0 main@%sm2_0))
                (=> (and main@_bb_0 main@entry_0) (= main@%.02.i_0 0))
                (=> (and main@_bb_0 main@entry_0) (= main@%.0.i_0 0))
                (=> (and main@_bb_0 main@entry_0)
                    (= main@%shadow.mem.0.0_1 main@%shadow.mem.0.0_0))
                (=> (and main@_bb_0 main@entry_0)
                    (= main@%.02.i_1 main@%.02.i_0))
                (=> (and main@_bb_0 main@entry_0) (= main@%.0.i_1 main@%.0.i_0))
                main@_bb_0)))
  (=> a!1
      (main@_bb main@%_0_0
                main@%.02.i_1
                main@%.0.i_1
                main@%shadow.mem.0.0_1
                main@%_2_0))))
(rule (let ((a!1 (=> main@_bb3_0
               (= main@%_8_0 (+ main@%_0_0 (* 0 4000000) (* main@%.02.i_0 4)))))
      (a!2 (=> main@_bb4_0
               (= main@%_13_0 (+ main@%_0_0 (* 0 4000000) (* main@%.02.i_0 4)))))
      (a!3 (=> main@_bb5_0
               (= main@%_15_0 (+ main@%_0_0 (* 0 4000000) (* main@%.02.i_0 4))))))
(let ((a!4 (and (main@_bb main@%_0_0
                          main@%.02.i_0
                          main@%.0.i_0
                          main@%shadow.mem.0.0_0
                          main@%_2_0)
                true
                (= main@%_5_0 (< main@%.02.i_0 1000000))
                (=> main@_bb3_0 (and main@_bb3_0 main@_bb_0))
                (=> (and main@_bb3_0 main@_bb_0) main@%_5_0)
                (=> main@_bb3_0 (= main@%_7_0 (+ main@%.02.i_0 1)))
                a!1
                (=> main@_bb3_0 (or (<= main@%_0_0 0) (> main@%_8_0 0)))
                (=> main@_bb3_0 (> main@%_0_0 0))
                (=> main@_bb3_0
                    (= main@%sm_0
                       (store main@%shadow.mem.0.0_0 main@%_8_0 main@%_7_0)))
                (=> main@_bb3_0 (= main@%_9_0 (> main@%.02.i_0 500000)))
                (=> main@_bb3_0 (= main@%_10_0 (< main@%.0.i_0 1)))
                (=> main@_bb3_0
                    (= main@%or.cond.i_0 (and main@%_9_0 main@%_10_0)))
                (=> main@_bb4_0 (and main@_bb4_0 main@_bb3_0))
                (=> (and main@_bb4_0 main@_bb3_0) main@%or.cond.i_0)
                a!2
                (=> main@_bb4_0 (or (<= main@%_0_0 0) (> main@%_13_0 0)))
                (=> main@_bb4_0 (> main@%_0_0 0))
                (=> main@_bb4_0
                    (= main@%sm1_0 (store main@%sm_0 main@%_13_0 main@%_12_0)))
                (=> |tuple(main@_bb3_0, main@_bb5_0)| main@_bb3_0)
                (=> main@_bb5_0
                    (or (and main@_bb5_0 main@_bb4_0)
                        |tuple(main@_bb3_0, main@_bb5_0)|))
                (=> |tuple(main@_bb3_0, main@_bb5_0)| (not main@%or.cond.i_0))
                (=> (and main@_bb5_0 main@_bb4_0)
                    (= main@%shadow.mem.0.1_0 main@%sm1_0))
                (=> |tuple(main@_bb3_0, main@_bb5_0)|
                    (= main@%shadow.mem.0.1_1 main@%sm_0))
                (=> (and main@_bb5_0 main@_bb4_0)
                    (= main@%shadow.mem.0.1_2 main@%shadow.mem.0.1_0))
                (=> |tuple(main@_bb3_0, main@_bb5_0)|
                    (= main@%shadow.mem.0.1_2 main@%shadow.mem.0.1_1))
                a!3
                (=> main@_bb5_0 (or (<= main@%_0_0 0) (> main@%_15_0 0)))
                (=> main@_bb5_0 (> main@%_0_0 0))
                (=> main@_bb5_0
                    (= main@%_16_0 (select main@%shadow.mem.0.1_2 main@%_15_0)))
                (=> main@_bb5_0 (= main@%_17_0 (= main@%_16_0 0)))
                (=> main@_bb5_0 (= main@%_18_0 (ite main@%_17_0 1 0)))
                (=> main@_bb5_0 (= main@%..0.i_0 (+ main@%.0.i_0 main@%_18_0)))
                (=> main@_bb5_0 (= main@%_19_0 (+ main@%.02.i_0 1)))
                (=> main@_bb_1 (and main@_bb_1 main@_bb5_0))
                (=> (and main@_bb_1 main@_bb5_0)
                    (= main@%shadow.mem.0.0_1 main@%shadow.mem.0.1_2))
                (=> (and main@_bb_1 main@_bb5_0) (= main@%.02.i_1 main@%_19_0))
                (=> (and main@_bb_1 main@_bb5_0) (= main@%.0.i_1 main@%..0.i_0))
                (=> (and main@_bb_1 main@_bb5_0)
                    (= main@%shadow.mem.0.0_2 main@%shadow.mem.0.0_1))
                (=> (and main@_bb_1 main@_bb5_0)
                    (= main@%.02.i_2 main@%.02.i_1))
                (=> (and main@_bb_1 main@_bb5_0) (= main@%.0.i_2 main@%.0.i_1))
                main@_bb_1)))
  (=> a!4
      (main@_bb main@%_0_0
                main@%.02.i_2
                main@%.0.i_2
                main@%shadow.mem.0.0_2
                main@%_2_0)))))
(rule (=> (and (main@_bb main@%_0_0
                   main@%.02.i_0
                   main@%.0.i_0
                   main@%shadow.mem.0.0_0
                   main@%_2_0)
         true
         (= main@%_5_0 (< main@%.02.i_0 1000000))
         (=> main@_bb6_0 (and main@_bb6_0 main@_bb_0))
         (=> (and main@_bb6_0 main@_bb_0) (not main@%_5_0))
         (=> main@_bb7_0 (and main@_bb7_0 main@_bb6_0))
         (=> (and main@_bb7_0 main@_bb6_0) (= main@%.04.i_0 0))
         (=> (and main@_bb7_0 main@_bb6_0) (= main@%.13.i_0 0))
         (=> (and main@_bb7_0 main@_bb6_0) (= main@%.04.i_1 main@%.04.i_0))
         (=> (and main@_bb7_0 main@_bb6_0) (= main@%.13.i_1 main@%.13.i_0))
         main@_bb7_0)
    (main@_bb7 main@%_0_0 main@%shadow.mem.0.0_0 main@%.13.i_1 main@%.04.i_1)))
(rule (let ((a!1 (=> main@_bb8_0
               (= main@%_24_0 (+ main@%_0_0 (* 0 4000000) (* main@%.13.i_0 4))))))
(let ((a!2 (and (main@_bb7 main@%_0_0
                           main@%shadow.mem.0.0_0
                           main@%.13.i_0
                           main@%.04.i_0)
                true
                (= main@%_22_0 (< main@%.13.i_0 1000000))
                main@%_22_0
                (=> main@_bb8_0 (and main@_bb8_0 main@_bb7_0))
                a!1
                (=> main@_bb8_0 (or (<= main@%_0_0 0) (> main@%_24_0 0)))
                (=> main@_bb8_0 (> main@%_0_0 0))
                (=> main@_bb8_0
                    (= main@%_25_0 (select main@%shadow.mem.0.0_0 main@%_24_0)))
                (=> main@_bb8_0 (= main@%_26_0 (= main@%_25_0 0)))
                (=> main@_bb8_0 (= main@%_27_0 (ite main@%_26_0 1 0)))
                (=> main@_bb8_0
                    (= main@%..04.i_0 (+ main@%.04.i_0 main@%_27_0)))
                (=> main@_bb8_0 (= main@%_28_0 (< main@%..04.i_0 2)))
                (=> main@_bb8_0 (= main@%_29_0 (ite main@%_28_0 1 0)))
                (=> main@postcall_0 (and main@postcall_0 main@_bb8_0))
                (=> (and main@postcall_0 main@_bb8_0) main@%_30_0)
                (__JVERIFIER_assert main@postcall_0 false false main@%_29_0)
                (=> main@postcall_0 (= main@%_31_0 (+ main@%.13.i_0 1)))
                (=> main@_bb7_1 (and main@_bb7_1 main@postcall_0))
                (=> (and main@_bb7_1 main@postcall_0)
                    (= main@%.04.i_1 main@%..04.i_0))
                (=> (and main@_bb7_1 main@postcall_0)
                    (= main@%.13.i_1 main@%_31_0))
                (=> (and main@_bb7_1 main@postcall_0)
                    (= main@%.04.i_2 main@%.04.i_1))
                (=> (and main@_bb7_1 main@postcall_0)
                    (= main@%.13.i_2 main@%.13.i_1))
                main@_bb7_1)))
  (=> a!2
      (main@_bb7 main@%_0_0 main@%shadow.mem.0.0_0 main@%.13.i_2 main@%.04.i_2)))))
(rule (let ((a!1 (=> main@_bb8_0
               (= main@%_24_0 (+ main@%_0_0 (* 0 4000000) (* main@%.13.i_0 4))))))
(let ((a!2 (and (main@_bb7 main@%_0_0
                           main@%shadow.mem.0.0_0
                           main@%.13.i_0
                           main@%.04.i_0)
                true
                (= main@%_22_0 (< main@%.13.i_0 1000000))
                main@%_22_0
                (=> main@_bb8_0 (and main@_bb8_0 main@_bb7_0))
                a!1
                (=> main@_bb8_0 (or (<= main@%_0_0 0) (> main@%_24_0 0)))
                (=> main@_bb8_0 (> main@%_0_0 0))
                (=> main@_bb8_0
                    (= main@%_25_0 (select main@%shadow.mem.0.0_0 main@%_24_0)))
                (=> main@_bb8_0 (= main@%_26_0 (= main@%_25_0 0)))
                (=> main@_bb8_0 (= main@%_27_0 (ite main@%_26_0 1 0)))
                (=> main@_bb8_0
                    (= main@%..04.i_0 (+ main@%.04.i_0 main@%_27_0)))
                (=> main@_bb8_0 (= main@%_28_0 (< main@%..04.i_0 2)))
                (=> main@_bb8_0 (= main@%_29_0 (ite main@%_28_0 1 0)))
                (=> main@precall_0 (and main@precall_0 main@_bb8_0))
                (=> (and main@precall_0 main@_bb8_0) (not main@%_30_0))
                (=> main@__JVERIFIER_assert_0
                    (and main@__JVERIFIER_assert_0 main@precall_0))
                (=> main@__JVERIFIER_assert_0 (= main@%_32_0 (= main@%_29_0 0)))
                (=> main@__JVERIFIER_assert_0 main@%_32_0)
                (=> main@_bb9_0 (and main@_bb9_0 main@__JVERIFIER_assert_0))
                (=> main@verifier.error_0
                    (and main@verifier.error_0 main@_bb9_0))
                (=> main@verifier.error.split_0
                    (and main@verifier.error.split_0 main@verifier.error_0))
                main@verifier.error.split_0)))
  (=> a!2 main@verifier.error.split))))
(query main@verifier.error.split)

